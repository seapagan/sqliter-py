...
class TestQuery:
    ...
    def test_order_default(self, db_mock) -> None:
    ...
    def test_order_with_reverse_true(self, db_mock) -> None:
        """Test the order method works with reverse=True (descending order)."""
        ...
        # Descending order
        results = (
            db_mock.select(TestModel).order("name", reverse=True).fetch_all()
        )
        assert len(results) == 3
    ...
    def test_order_with_both_reverse_and_direction(self, db_mock) -> None:
        """Test both 'direction' and 'reverse' raises an InvalidOrderError."""
        ...
        with pytest.raises(
            ...
            db_mock.select(TestModel).order(
